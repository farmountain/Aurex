...
//!
//! This module mirrors the SYCL backend in the amduda crate but keeps the
//! implementation lightweight so that the `aurex-backend` crate can compile
//! without the oneAPI runtime.  Device enumeration, kernel compilation and
//! launch are modelled but ultimately fall back to the [`CpuBackend`]
//! implementation.  This allows the dispatch layer to exercise the SYCL code
//! paths during tests while staying fully portable.
...
impl SyclBackend {
    ...
    pub fn enumerate() -> Vec<SyclDevice> {
        vec![SyclDevice { id: 0, name: "SYCL CPU" }]
    }
